...
import cn.mics.brank.tes.TestConfigHelper;
...
public class End2endIT {
	...
	public void setup() {
		...
		try {
			...
			final SampleOrg sampleOrg = testConfig.getIntegrationTestsSampleOrg("peerOrg1");
			CreatChannel constructChannel = new CreatChannel(FOO_CHANNEL_NAME, client, sampleOrg);
			constructChannel.constructChannel();
			Channel fooChannel = constructChannel.getNewChannel();
			...
			ChaincodeID chaincodeID = runChannel.getChaincodeID();
			Collection<ProposalResponse> successful = runChannel.getSuccessful();
			System.out.println("++++++" + successful);
			Collection<ProposalResponse> failed = runChannel.getFailed();
			Out.out("\n");
			// invoke and query
